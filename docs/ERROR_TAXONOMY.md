...
line, and pie charts. It does not grade grammar, vocabulary, style, or an overall
IELTS band.
...
Every statistical chart response contains an `error_taxonomy` object and uses
chart schema version `1.1`:
...
test image, intentionally flawed report, expected expert annotations, and exact
steps are documented in `test_samples/ERROR_TAXONOMY_WORKFLOW.md`. The image must
go through DePlot and the report must go through DeepSeek extraction/alignment;
the expected labels are never included in the runtime request.
